...
1. **内网网段为 `192.168.10.0/24`，且在多个文件硬编码**：`bridges.nix`（宿主机 IP/网关）、`samba.nix`（hosts allow）、`nfs.nix`（exports）、`syncthing.nix`（guiAddress）、`music.nix`（Navidrome/Feishin 绑定地址）、`microvm.router.vmIp` 选项（router-image 模块，deploy 脚本 ssh 目标）、**router-image 仓库的 `network.env`**（VM 网络参数权威源，含 TS_ADVERTISE_ROUTES 与 LAN_GATEWAY 浮动网关）、**yunshu 容器参数**（`modules/services/yunshu.nix` 的 lanAddress/upstreamGateway/floatIp）。修改网段时必须全局同步这些位置，否则服务绑定错 IP 或防火墙/共享拒绝访问。
